...
const LOCAL_STORAGE_KEY = 'waldorf-astoria-tier-list.hotels';
const TOP_PICKS_STORAGE_KEY = 'waldorf-astoria-tier-list.top-picks';
const TOP_SUITES_STORAGE_KEY = 'waldorf-astoria-tier-list.top-room-highlights';
const TOP_FUTURE_STAYS_STORAGE_KEY = 'waldorf-astoria-tier-list.top-future-stays';
const TOP_EXPERIENCES_STORAGE_KEY = 'waldorf-astoria-tier-list.top-experiences';
...
> = {
  S: {
    panel: 'from-[#123a63]/18 via-[#dbe5f0] to-white',
    badge: 'bg-[#123a63] text-white',
    empty: 'Nothing in S-tier yet.'
  },
  A: {
    panel: 'from-[#315f8f]/16 via-[#e3ebf5] to-white',
    badge: 'bg-[#315f8f] text-white',
    empty: 'No A-tier entries yet.'
  },
  B: {
    panel: 'from-[#6580a0]/16 via-[#ebf1f6] to-white',
    badge: 'bg-[#6580a0] text-white',
    empty: 'No B-tier entries yet.'
  },
  C: {
    panel: 'from-[#8d9db1]/16 via-[#eff3f7] to-white',
    badge: 'bg-[#8d9db1] text-white',
    empty: 'No C-tier entries yet.'
  },
  D: {
    panel: 'from-[#a8b5c4]/18 via-[#f4f7fa] to-white',
    badge: 'bg-[#a8b5c4] text-[rgb(var(--page-foreground))]',
...
export function WaldorfAstoriaTierListClient({
  initialHotels,
  ...
  const exploredHotelsWithSuites = useMemo(
    () => exploredHotels.filter((hotel) => hotel.roomEntries.length > 0),
    [exploredHotels]
  );
      ...
      sortHotelsByTier(
        exploredHotels.filter(
          (hotel) =>
            ...
            hotel.roomEntries.some((entry) => entry.withKelly)
        )
      ),
    [exploredHotels]
  );
  ...
  const kellySuiteCount = useMemo(
    () => kellyExplorationHotels.reduce((total, hotel) => total + hotel.roomEntries.filter((entry) => entry.withKelly).length, 0),
    [kellyExplorationHotels]
  );
  ...
  const menuSections: DashboardMenuSection[] = [
    ...
    },
    {
      ...
      label: 'My Waldorf Astoria Stays with Kelly ❤️❤️',
  ...
  async function handleSaveTopSuites() {
    ...
    if (selectedSuites.length !== new Set(selectedSuites).size) {
      setTopSuitesError('Each top room type must be a different hotel and room combination.');
      return;
    }
    ...
    try {
      ...
      setTopSuitesError(error instanceof Error ? error.message : 'Unable to save top 3 room types.');
      return;
    }
  }
      ...
      <div className="relative mx-auto flex max-w-[1500px] flex-col gap-4 pb-8">
        ...
        <section className="glass-panel rounded-[28px] px-4 py-4 sm:px-5 sm:py-5" style={{ order: getSectionOrder('topSuites') }}>
          ...
          {hasCompleteTopSuites ? (
            ...
            <div className="mt-5 rounded-[22px] border border-dashed border-[rgba(0,102,179,0.16)] bg-white/58 p-5 text-sm text-[rgba(34,58,86,0.62)]">
              Click Top 3 Room Types to set up your favorite room highlights.
            </div>
          )}
        </section>
        ) : null}
        ...
        >
          ...
          {activeSuiteSlide ? (
              ...
              <article className="overflow-hidden rounded-[36px] border border-white/8 bg-[#0a121b] shadow-[0_28px_80px_rgba(8,25,43,0.38)]">
                ...
                >
                  ...
                  <button
                    ...
                    aria-label="Show previous room highlight"
                  >
                    ←
                  </button>
                  ...
                  <button
                    ...
                    aria-label="Show next room highlight"
                  >
                    →
                  </button>
            ...
            <div className="mt-5 rounded-[22px] border border-dashed border-[rgba(0,102,179,0.16)] bg-white/58 p-5 text-sm text-[rgba(34,58,86,0.62)]">
              Add room types with an image URL to explored hotels and they will appear here.
            </div>
          )}
        </section>
        ) : null}
          ...
          <div className="flex items-center justify-between gap-3">
            <div>
              <button
                ...
                className="section-label transition hover:text-[rgb(var(--wine))]"
              >
                My Waldorf Astoria Bucket List
              </button>
            </div>
        ...
        <section className="glass-panel rounded-[28px] px-4 py-4 sm:px-5 sm:py-5" style={{ order: getSectionOrder('kellyExplorations') }}>
          <div>
            <p className="section-label">My Waldorf Astoria Stays with Kelly ❤️❤️</p>
          </div>
      ...
      {isSuitePickerOpen ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-[rgba(42,18,22,0.48)] px-4 py-8 backdrop-blur-sm">
          <div className="glass-panel max-h-[calc(100vh-3rem)] w-full max-w-5xl overflow-hidden rounded-[30px] p-5 sm:p-7">
            <div className="flex items-start justify-between gap-4">
              <div>
                <p className="section-label">My Room Highlights</p>
              </div>
              ...
              <button
                ...
                className="inline-flex h-10 w-10 items-center justify-center rounded-full border border-[rgba(118,31,47,0.16)] bg-white/80 text-lg text-[rgb(var(--wine))] transition hover:bg-[rgba(118,31,47,0.06)]"
                aria-label="Close room highlights"
              >
                ×
              </button>
            </div>
            ...
            <div className="mt-6 max-h-[70vh] overflow-auto pr-1">
              {loggedSuiteSlides.length ? (
                ...
                <div className="rounded-[22px] border border-dashed border-[rgba(0,102,179,0.16)] bg-white/58 p-5 text-sm text-[rgba(34,58,86,0.62)]">
                  Add room types with image URLs to explored hotels and they will appear here.
                </div>
              )}
            </div>
          </div>
      ...
      {isBucketListPickerOpen ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-[rgba(42,18,22,0.48)] px-4 py-8 backdrop-blur-sm">
          <div className="glass-panel max-h-[calc(100vh-3rem)] w-full max-w-5xl overflow-hidden rounded-[30px] p-5 sm:p-7">
            <div className="flex items-start justify-between gap-4">
              <div>
                <p className="section-label">My Waldorf Astoria Bucket List</p>
              </div>
            ...
            <div className="mt-6 max-h-[70vh] overflow-auto pr-1">
              {bucketListHotels.length ? (
                ...
                <div className="rounded-[22px] border border-dashed border-[rgba(0,102,179,0.16)] bg-white/58 p-5 text-sm text-[rgba(34,58,86,0.62)]">
                  Add a bucket-list hotel first, then click My Waldorf Astoria Bucket List to edit it from here.
                </div>
              )}
            </div>
          </div>
          ...
          <div className="glass-panel w-full max-w-4xl rounded-[30px] p-5 sm:p-7">
            <div className="flex items-start justify-between gap-4">
              <div>
                ...
                <h2 className="mt-2 text-2xl font-semibold leading-none text-[rgb(var(--page-foreground))] font-[family:var(--font-display)] sm:text-4xl">
                  Waldorf Astoria stays with Kelly ❤️❤️
                </h2>
              </div>
            ...
            <div className="mt-6 max-h-[70vh] overflow-auto pr-1">
              {BRANDS_BY_SEGMENT.length ? (
                ...
                <div className="rounded-[22px] border border-dashed border-[rgba(0,102,179,0.16)] bg-white/58 p-5 text-sm text-[rgba(34,58,86,0.62)]">
                  Mark a stay or room highlight with Kelly ❤️ to start building this brand palette.
                </div>
              )}
            </div>
          </div>
          ...
          <div className="glass-panel w-full max-w-3xl rounded-[30px] p-5 sm:p-7">
            <div className="flex items-start justify-between gap-4">
              <div>
                ...
                <h2 className="mt-2 text-2xl font-semibold leading-none text-[rgb(var(--page-foreground))] font-[family:var(--font-display)] sm:text-4xl">
                  Planned Waldorf Astoria Hotels
                </h2>
              </div>
              <button
                type="button"
      ...
      {isTopSuitesOpen ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-[rgba(42,18,22,0.48)] px-4 py-8 backdrop-blur-sm">
          <div className="glass-panel max-h-[calc(100vh-3rem)] w-full max-w-4xl overflow-y-auto rounded-[30px] p-5 sm:p-7">
            <div className="flex items-start justify-between gap-4">
              <div>
                <p className="section-label">Top 3 Room Types</p>
                <h2 className="mt-2 text-2xl font-semibold leading-none text-[rgb(var(--page-foreground))] font-[family:var(--font-display)] sm:text-4xl">
                  Build your favorites
                </h2>
                <div className="mt-2 text-sm text-[rgba(34,58,86,0.62)]">
                  Pick your three favorite room types, add the room name, and optionally paste a photo URL.
                </div>
              </div>
              <button
                type="button"
                ...
                className="inline-flex h-10 w-10 items-center justify-center rounded-full border border-[rgba(118,31,47,0.16)] bg-white/80 text-lg text-[rgb(var(--wine))] transition hover:bg-[rgba(118,31,47,0.06)]"
                aria-label="Close top 3 room types"
              >
                ×
              </button>
            </div>
                ...
                return (
                  <div key={slot.rank} className="rounded-[24px] border border-[rgba(0,102,179,0.1)] bg-white/68 p-4">
                    ...
                    <div className="grid gap-3 md:grid-cols-3">
                      ...
                      <label className="space-y-2">
                        <span className="text-sm font-semibold text-[rgb(var(--page-foreground))]">Room type</span>
                        <>
                          <input
                ...
                <div className="space-y-3 md:col-span-2">
                  <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                    <div>
                      ...
                      <div className="mt-1 text-xs text-[rgba(34,58,86,0.58)]">
                        {draft.stayType === 'EXPLORED'
                          ? 'Add each room type you stayed in for this hotel.'
                          : 'Add planned room types for this hotel.'}
                      </div>
                    </div>
                    ...
                    <div className="space-y-3">
                      {draft.roomEntries.map((entry, index) => (
                        <div
                          ...
                          className="grid gap-3 rounded-[20px] border border-[rgba(0,102,179,0.1)] bg-white/72 p-3 md:grid-cols-[minmax(0,1fr)_auto]"
                        >
                          <input
                            value={entry.label}
                            onChange={(event) =>
                          ...
                          <label className="md:col-span-2 flex items-center justify-between gap-4 rounded-[16px] border border-[rgba(0,102,179,0.1)] bg-white/72 px-4 py-3">
                            <div>
                              ...
                              <div className="mt-1 text-xs text-[rgba(34,58,86,0.58)]">
                                Turn this on for room highlights that were part of a Kelly trip.
                              </div>
                            </div>
